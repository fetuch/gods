...
server.post("/gods/", function(req, res, next) {
  const error = validateGod(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate a slug for new gods.
    next();
  }
});
...
function validateGod(god) {
  if (!god.name) return "Name is required.";
  if (!god.description) return "Description is required.";
  if (!god.domain) return "Domain is required.";
  return "";
}
